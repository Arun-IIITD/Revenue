...
def plot_revenue(actual_dates, actual_revenue, predicted_dates, predicted_revenue, title):
    ...
    fig.update_layout(
        ...
        yaxis=dict(title='Revenue (in Lakhs)', range=[0, max(max(actual_revenue), max(predicted_revenue)) + 100]),
        hovermode='x',
...
def main():
    ...
    with col2:
        ...
        st.write(f"Sensitivity: {round(mean(sensitivity_values_for_14_days),3)}%")
        st.markdown("---")
    ...
    with col1:
        ...
        st.write(f"Sensitivity: {round(mean(sensitivity_values_for_21_days),3)}%")
        st.markdown("---")
